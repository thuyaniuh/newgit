...
import { Card, Paragraph, TextInput, Title, Button } from "react-native-paper";
                ...
                <Card style={styles.card}>
                    <View style={styles.cardContent}>
                        <Title style={styles.title}>
                            {item.project?.name || "Dự án không xác định"}
                        </Title>
                        <Paragraph>
                            Ngày: {item.date || "Không có ngày"}
                        </Paragraph>
                        <Paragraph>
                            Nhà cung cấp:{" "}
                            {item.supplier
                        ...
                        <Paragraph>
                            Người dùng:{" "}
                            {item.user?.name || "Không có thông tin người dùng"}
                        </Paragraph>
                        <Paragraph>
                            Tổng giá:{" "}
                            {item.total_price
                                ? `${item.total_price.toString()} VND`
                                : "0 VND"}
                        </Paragraph>
                    </View>
                </Card>
    ...
    return (
        <View style={styles.container}>
            <Button
                ...
                onPress={() => navigation.navigate("AddPurchaseScreen")}
            >
                Thêm Mới
            </Button>
            {purchases && purchases.length > 0 ? (
                <FlatList
                    ...
                    keyExtractor={(item) =>
                        item.purchase_id?.toString() || Math.random().toString()
                    }
                />
            ) : (
